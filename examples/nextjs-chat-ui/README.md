...
8. When a live `chat.user.message` event arrives, that listener reads `await session.all()`, runs `streamText(...)`, and appends assistant chunks back into the same session.
